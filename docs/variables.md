...
again whenever a value changes. An action that sets values, a form, a
choice, an `env-set` or a capture, is not finished until the open
terminals have loaded them, so the next action clicked runs against the
new values. The load takes its turn with the commands a terminal is
sent: it waits for one still running, and the next one waits for it. The
same file sets the [workshop prompt](platforms.md#the-workshop-prompt).
Under JupyterLite the terminal cannot source a file, so the values are
sent as `export` commands instead. A manifest `env` mapping adds fixed
