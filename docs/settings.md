...
| `panelSide`          | string | `right`      | Which sidebar the panel starts in, `left` or `right`. Dragging the panel to the other side is remembered by JupyterLab's layout, and a workshop whose manifest sets `instructions.side` overrides it. |
