...
| -------------------- | ------ | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `defaultWorkshop`    | string | `examples/git-basics` | Directory of the workshop to open when JupyterLab starts and no workshop was open before, relative to the JupyterLab root. Empty opens nothing. The shipped default points at the example in the project's own checkout. |
...
| `registries`         | list   | the project's index   | Registry index files whose workshops the browser offers: `http(s)` URLs, or paths relative to the JupyterLab root. The default is the index of this project's examples.                                                  |
...
| `disabledFeatures` | list    | `[]`    | Parts of the extension to remove, from `open-directory`, `open-url`, `registries`, `available`, `remove`, `close`, `browse` and `author`. See [locking down a deployment](deploying.md#locking-down-a-deployment). |
