...
| `script`         | None; name the file with `:script:`          | On the server as a subprocess with the workshop directory as its working directory. Exit code 0 passes. Python files run with the server's interpreter; other files must be executable.                                                |
...
`:timeout:` bounds script runs. Code substrates need the `kernel-exec`
capability; the `contents` and `ui` substrates need nothing. Under the
